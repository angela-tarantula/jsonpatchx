...
- Examples:
  - `dependency.yml` requests `pull-requests: write` to post review summaries.
...
## Dependency Review (`dependency.yml`)
...
- `allow-dependencies-licenses` is a version-pinned exception list for
  dependencies where license detection is currently unknown.
