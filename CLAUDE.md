...
- [CONTRIBUTING.md](CONTRIBUTING.md) — full local setup; the commands below are
  a quick reference.
...
`pyproject.toml` sets `filterwarnings = ["error"]` — any unexpected warning
fails the suite. Coverage `fail_under = 90`.
...
7. **`pydantic.py`** — `JsonPatchFor[T]`: a `JsonPatch` parameterized by a
   target Pydantic model so the patched document round-trips through `T`'s
   validation.
