...
| `422`  | FastAPI request validation and `InvalidJSONPointer` / `InvalidJSONSelector` cover invalid patch documents and invalid pointer syntax.                                 |
| `409`  | `PatchConflictError` means the patch cannot be applied to the current resource state. `PatchValidationError` means the patched result fails the target model schema.  |
